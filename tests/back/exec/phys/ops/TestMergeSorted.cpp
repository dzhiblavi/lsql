#include "tests/back/exec/phys/MockOperation.h"
#include "tests/back/exec/phys/ops/OperationTest.h"
...
struct MergeSortedTest : OperationTest {
    MergeSortedTest() {
        auto op = arc<MergeSorted>(
            0, std::vector<Arc<Scalar>>{arc<IdentifierScalar>(0, 0, ValueType::Integer)}, false);

        left->output(op->subLeft());
        right->output(op->subRight());
        setOperation(op);
    }

    std::shared_ptr<MockOperation> left = arc<MockOperation>();
    std::shared_ptr<MockOperation> right = arc<MockOperation>();
};
