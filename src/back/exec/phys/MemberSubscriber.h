...
class MemberSubscriber : public Subscriber, util::Pinned {
 public:
    using MethodType = bool (Self::*)(const Record*);
    ...
    bool consume(const Record* record) override {
        [[maybe_unused]] auto scope = consumeScope();
        auto _ = handle_.scope();
        return (self_->*method_)(record);
    }

    prof::ScopeHandleBase scopeHandle() const override { return handle_; }
